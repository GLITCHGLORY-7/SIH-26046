...
def create_ethics_submission(
    ...
    current_user: User = Depends(require_any_permission("ethics.submit", "ethics.create", "trial.edit")),
    db: Session = Depends(get_db)
):
...
def record_ethics_decision(
    ...
    """Record IEC ethical decision (Approved, Rejected, Modifications Requested)."""
...
def update_ctri_registration(
    ...
    current_user: User = Depends(require_any_permission("regulatory.manage", "trial.edit", "ethics.approve")),
    db: Session = Depends(get_db)
):
...
def list_regulatory_events(
    trial_id: Optional[int] = Query(None, description="Filter by clinical trial ID"),
    current_user: User = Depends(require_any_permission("trial.view", "regulatory.manage", "ethics.view")),
    db: Session = Depends(get_db)
):
...
def create_regulatory_event(
    ...
    current_user: User = Depends(require_any_permission("regulatory.manage", "trial.edit", "ethics.approve", "ethics.view")),
    db: Session = Depends(get_db)
):
