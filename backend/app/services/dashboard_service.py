...
class DashboardService:
    def get_realtime_metrics(
        ...
        pending_ethics_reviews_count = db.query(EthicsSubmission).filter(
            EthicsSubmission.status.in_(["Submitted", "Under Review", "Changes Required"])
        ).count()

        active_ethics_clearances_count = db.query(EthicsSubmission).filter(
            EthicsSubmission.status == "Approved"
        ).count()

        open_sae_count = sum(1 for e in all_aes if e.is_serious and e.status != "Closed")

        pending_dcgi_notices_count = db.query(AdverseEvent).filter(
            AdverseEvent.is_serious.is_(True),
            AdverseEvent.expedited_notified_at.is_(None)
        ).count()
        ...
        return DashboardMetricsResponse(
            ...
            pending_ethics_reviews_count=pending_ethics_reviews_count,
            active_ethics_clearances_count=active_ethics_clearances_count,
            open_sae_count=open_sae_count,
            pending_dcgi_notices_count=pending_dcgi_notices_count,
    ...
    def get_realtime_alerts(self, db: Session, current_user: User) -> List[DashboardAlertItem]:
        ...
        user_role = current_user.role.name if current_user.role else "ADMIN"

        # Alert 1: Overdue Clinical Visits (Restricted: ONLY operational site teams and monitors)
        # Ethics Committee, PV Officers, and Regulators are restricted from operational visit reminders under GCP
        if user_role in ["ADMIN", "PRINCIPAL_INVESTIGATOR", "STUDY_COORDINATOR", "CLINICAL_TRIAL_MONITOR"]:
        ...
        # Required by: PV Lead (urgent causality), Ethics Committee (participant risk/benefit), PI, and Regulator
        if user_role in ["ADMIN", "PHARMACOVIGILANCE_OFFICER", "PRINCIPAL_INVESTIGATOR", "ETHICS_COMMITTEE", "REGULATOR"]:
        ...
        # Required by: Ethics Committee, PI, Regulator
        if user_role in ["ADMIN", "ETHICS_COMMITTEE", "PRINCIPAL_INVESTIGATOR", "REGULATOR"]:
            ...
            for eth in expiring_ethics:
                ...
                alerts.append(
                    DashboardAlertItem(
                        ...
                        action_url="/ethics-regulatory"
                    )
                )

        # Alert 4: Recruitment Lag Warning (Restricted: ONLY operational site teams and CRA)
        # Ethics and PV officers must not be pressured by recruitment pace
        if user_role in ["ADMIN", "PRINCIPAL_INVESTIGATOR", "STUDY_COORDINATOR", "CLINICAL_TRIAL_MONITOR"]:
        ...
        # Required by: Regulator, PI, PV Lead, Ethics Committee
        if user_role in ["ADMIN", "REGULATOR", "PRINCIPAL_INVESTIGATOR", "PHARMACOVIGILANCE_OFFICER", "ETHICS_COMMITTEE"]:
        ...
        # Alert 6: Role-Tailored Alert for Ethics Committee - Pending Protocol Dossiers
        if user_role in ["ADMIN", "ETHICS_COMMITTEE"]:
            pending_ethics = db.query(EthicsSubmission).filter(
                EthicsSubmission.status.in_(["Submitted", "Under Review", "Draft"])
            ).limit(2).all()

            for p_eth in pending_ethics:
                t_str = p_eth.trial.trial_id if p_eth.trial else f"Trial #{p_eth.trial_id}"
                alerts.append(
                    DashboardAlertItem(
                        id=f"alert-iec-pending-{p_eth.id}",
                        category="ETHICS",
                        severity="WARNING",
                        title="Protocol Dossier Awaiting IEC Review",
                        message=f"Trial '{t_str}' (Version {p_eth.protocol_version}) submitted on {p_eth.submission_date} is awaiting committee review and clearance sign-off.",
                        entity_type="ETHICS",
                        entity_id=str(p_eth.id),
                        timestamp=datetime.now(timezone.utc) - timedelta(hours=3),
                        action_url="/ethics-regulatory"
                    )
                )

        # Alert 7: Role-Tailored Alert for Pharmacovigilance - NDCTR 2019 Rule 42 24h Expedited Notice
        if user_role in ["ADMIN", "PHARMACOVIGILANCE_OFFICER"]:
            pending_saes = db.query(AdverseEvent).filter(
                AdverseEvent.is_serious.is_(True),
                AdverseEvent.expedited_notified_at.is_(None)
            ).limit(2).all()

            for p_sae in pending_saes:
                alerts.append(
                    DashboardAlertItem(
                        id=f"alert-sae-dcgi-{p_sae.id}",
                        category="SAFETY",
                        severity="CRITICAL",
                        title="NDCTR 2019 Rule 42: Expedited DCGI Notice Pending",
                        message=f"SAE '{p_sae.ae_id}' ({p_sae.event_term}) requires 24-hour statutory electronic notice to Central Licensing Authority (DCGI).",
                        entity_type="SAFETY",
                        entity_id=p_sae.ae_id,
                        timestamp=datetime.now(timezone.utc) - timedelta(minutes=20),
                        action_url=f"/safety/{p_sae.id}"
                    )
                )
