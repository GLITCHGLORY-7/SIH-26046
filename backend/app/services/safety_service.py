...
class SafetyService:
    def _generate_ae_id(self, db: Session, is_serious: bool = False) -> str:
        year = date.today().year
        count = db.query(AdverseEvent).count() + 1
        prefix = "SAE" if is_serious else "AE"
    ...
    def get_adverse_event(self, db: Session, ae_id: int) -> AdverseEventResponse:
        ...
        return AdverseEventResponse(
            ...
            reviewed_by=e.reviewed_by,
            closed_at=e.closed_at,
            expedited_notified_at=e.expedited_notified_at,
            signature_metadata=e.signature_metadata,
            created_at=e.created_at,
            updated_at=e.updated_at,
            participant_id_str=e.participant.participant_id if e.participant else None,
            trial_id_str=e.trial.trial_id if e.trial else None,
    ...
    def create_adverse_event(
        ...
        ae_code = self._generate_ae_id(db, is_serious=payload.is_serious)
    ...
    def review_adverse_event(
        ...
        if review_data.signature_metadata:
            ae.signature_metadata = review_data.signature_metadata
    ...
    def expedite_notify(
        ...
        ae.expedited_notified_at = datetime.now(timezone.utc)
        db.commit()
        db.refresh(ae)

        audit_service.record_event(
            db=db,
            action="EXPEDITED_SAE_NOTIFIED_TO_DCGI",
            entity_type="SAFETY",
            entity_id=ae.ae_id,
            description=f"Statutory 24-Hour Expedited SAE Notice dispatched for '{ae.ae_id}' to CDSCO / DCGI and Ethics Committee.",
            user_id=current_user.id,
            metadata={"ae_id": ae.ae_id, "trial_id": ae.trial_id, "timestamp": str(ae.expedited_notified_at)}
        )
        return ae
