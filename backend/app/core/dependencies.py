...
def require_any_permission(*permissions: str) -> Callable[[User], User]:
    """Dependency factory checking if current user has at least one of the specified permission(s)."""
    def permission_checker(current_user: User = Depends(get_current_user)) -> User:
        if not current_user.role:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        if not any(perm in user_perms for perm in permissions):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Access denied: Requires at least one permission from [{', '.join(permissions)}]."
            )
        return current_user
    return permission_checker
