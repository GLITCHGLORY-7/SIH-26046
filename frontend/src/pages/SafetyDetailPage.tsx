...
import type { AdverseEvent, AdverseEventReviewPayload, AECausality, AEOutcome, AEStatus, AuditLog, SignatureManifest } from '../types';
...
export const SafetyDetailPage: React.FC = () => {
  ...
  const [isDispatchingExpedited, setIsDispatchingExpedited] = useState(false);
  const [expeditedSuccessMessage, setExpeditedSuccessMessage] = useState<string | null>(null);
  ...
  const handleDispatchExpeditedNotice = async () => {
    if (!event) return;
    try {
      setIsDispatchingExpedited(true);
      setActionError(null);
      const updated = await safetyService.expeditedNotify(event.id);
      setEvent(updated);
      setExpeditedSuccessMessage('Mandatory 24-hour statutory regulatory notice successfully dispatched to DCGI & Ethics Committee.');
      try {
        const logs = await safetyService.getSafetyAuditLogs(updated.ae_id);
        setAuditLogs(logs);
      } catch (e) {}
    } catch (err: any) {
      setActionError(err?.response?.data?.detail || 'Failed to dispatch expedited notice. Please try again.');
    } finally {
      setIsDispatchingExpedited(false);
    }
  };

  const handleInitiateReviewSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!event) return;
    setIsSignatureModalOpen(true);
  };

  const handleSignatureComplete = async (manifest: SignatureManifest) => {
    ...
    try {
      ...
      const payload: AdverseEventReviewPayload = {
        ...
        signature_metadata: manifest
      };
      await safetyService.reviewEvent(event.id, payload);
      await loadData();
      setExpeditedSuccessMessage('Safety event review successfully certified and recorded under 21 CFR Part 11.');
    } catch (err: any) {
      setActionError(err?.response?.data?.detail || 'Failed to update safety event review');
    } finally {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      {expeditedSuccessMessage && (
        <div className="p-4 bg-emerald-50 border border-emerald-300 text-emerald-900 rounded-xl flex items-center justify-between shadow-sm animate-in fade-in">
          <div className="flex items-center space-x-2.5">
            <CheckCircle2 className="w-5 h-5 text-emerald-600 flex-shrink-0" />
            <span className="font-semibold text-xs sm:text-sm">{expeditedSuccessMessage}</span>
          </div>
          <button
            onClick={() => setExpeditedSuccessMessage(null)}
            className="text-emerald-700 hover:text-emerald-900 font-bold ml-4 text-xs px-2.5 py-1 rounded hover:bg-emerald-100 transition-colors"
          >
            ✕ Dismiss
          </button>
        </div>
      ...
      {actionError && (
        <div className="p-4 bg-rose-50 border border-rose-300 text-rose-900 rounded-xl flex items-center justify-between shadow-sm animate-in fade-in">
          <div className="flex items-center space-x-2.5">
            <AlertCircle className="w-5 h-5 text-rose-600 flex-shrink-0" />
            <span className="font-semibold text-xs sm:text-sm">{actionError}</span>
          </div>
          <button
            onClick={() => setActionError(null)}
            className="text-rose-700 hover:text-rose-900 font-bold ml-4 text-xs px-2.5 py-1 rounded hover:bg-rose-100 transition-colors"
          >
            ✕ Dismiss
          </button>
        </div>
      ...
      {/* 24-Hour Statutory Regulatory Notice to DCGI & IEC (NDCTR 2019 / CDSCO) */}
      {event.is_serious && (
        event.expedited_notified_at ? (
          <div className="bg-emerald-50 border border-emerald-300 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs">
            <div className="flex items-start space-x-3">
              <CheckCircle2 className="w-5 h-5 text-emerald-700 flex-shrink-0 mt-0.5" />
              <div>
                <div className="flex items-center space-x-2">
                  <h3 className="font-bold text-emerald-900 text-sm">
                    Statutory 24-Hour Expedited SAE Notice Dispatched to DCGI & IEC
                  </h3>
                ...
                <p className="text-emerald-800 mt-1">
                  Initial regulatory notification was transmitted on{' '}
                  <strong className="font-mono">{new Date(event.expedited_notified_at).toLocaleString()}</strong> in accordance with New Drugs and Clinical Trials Rules (NDCTR) 2019, Chapter VI, Rule 39.
                </p>
              </div>
            </div>
            <div className="flex items-center space-x-3 self-start sm:self-center">
              <span className="text-emerald-800 font-mono text-[11px] whitespace-nowrap bg-white px-2.5 py-1 rounded border border-emerald-200">
                Receipt: DCGI-EXP-{event.id}-ACK
              </span>
              {canReview && (
                <button
                  type="button"
                  onClick={handleDispatchExpeditedNotice}
                  disabled={isDispatchingExpedited}
                  className="inline-flex items-center space-x-1.5 px-3 py-1.5 bg-emerald-700 hover:bg-emerald-800 text-white font-semibold rounded-lg shadow-sm transition-colors text-xs whitespace-nowrap cursor-pointer"
                >
                  <Send className="w-3.5 h-3.5" />
                  <span>{isDispatchingExpedited ? 'Sending...' : 'Re-transmit Notice'}</span>
                </button>
              )}
            </div>
          </div>
        ) : (
          <div className="bg-white border-l-4 border-l-rose-600 border border-slate-200 rounded-xl p-5 text-xs shadow-sm space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-start space-x-3">
                <div className="p-2 rounded-lg bg-rose-50 text-rose-700 border border-rose-200 flex-shrink-0 mt-0.5">
                  <Clock className="w-5 h-5 text-rose-600 animate-pulse" />
                </div>
                <div>
                  <div className="flex items-center space-x-2">
                    <h3 className="font-bold text-slate-900 text-sm">
                      24-Hour Statutory Regulatory Notice to DCGI / CDSCO Pending
                    </h3>
                    <span className="px-2 py-0.5 bg-rose-100 text-rose-800 border border-rose-200 rounded font-mono font-bold text-[10px]">
                      NDCTR 2019 RULE 39
                    </span>
                  </div>
                  <p className="text-slate-600 mt-1 leading-relaxed">
                    Under <strong className="text-slate-800">NDCTR 2019 Chapter VI (Rule 39) & CDSCO Good Clinical Practice</strong>, every Serious Adverse Event (SAE) occurring in a clinical trial must be reported to the Drugs Controller General (India) and the Institutional Ethics Committee within <strong className="text-slate-900">24 hours</strong> of occurrence.
                  </p>
                </div>
              </div>
              {canReview && (
                <button
                  onClick={handleDispatchExpeditedNotice}
                  disabled={isDispatchingExpedited}
                  className="inline-flex items-center space-x-2 px-4 py-2.5 bg-rose-700 hover:bg-rose-800 disabled:opacity-50 text-white font-bold rounded-lg shadow-sm transition-all self-start sm:self-center whitespace-nowrap cursor-pointer"
                >
                  <Send className="w-4 h-4" />
                  <span>{isDispatchingExpedited ? 'Dispatching Notice...' : 'Dispatch 24-Hour Notice to DCGI'}</span>
                </button>
              )}
            </div>
          </div>
            ...
            {event.signature_metadata && (
              <div className="p-3 bg-emerald-50 rounded-lg border border-emerald-200 text-emerald-900 space-y-1">
                <div className="flex items-center space-x-1.5 font-bold text-xs text-emerald-800">
                  ...
                  <span>21 CFR Part 11 Electronic Signature Verified</span>
                </div>
                <p className="text-[11px] text-emerald-800">
                  Signer: <strong>{event.signature_metadata.signer_name}</strong> ({event.signature_metadata.signer_role})
                </p>
                <p className="text-[10px] font-mono text-emerald-700 truncate">
                  Digest: {event.signature_metadata.signature_hash || event.signature_metadata.sha256_hash}
                </p>
                <p className="text-[10px] font-mono text-emerald-600">
                  Certified: {event.signature_metadata.timestamp || event.signature_metadata.timestamp_iso}
                </p>
              </div>
            )}

            {canReview ? (
              <form onSubmit={handleInitiateReviewSubmit} className="space-y-4 text-xs">
                <div>
                ...
                <div className="p-2.5 bg-slate-50 border border-slate-200 rounded text-[11px] text-slate-600 flex items-center space-x-1.5">
                  <Lock className="w-3.5 h-3.5 text-rose-600 flex-shrink-0" />
                  <span>21 CFR Part 11 re-authentication required upon submission.</span>
                </div>

                <button
                  type="submit"
                  disabled={isSubmittingReview}
                  className="w-full inline-flex items-center justify-center space-x-2 py-2.5 bg-rose-600 hover:bg-rose-700 disabled:opacity-50 text-white rounded-lg font-bold text-xs shadow transition-colors"
                >
                  <Lock className="w-3.5 h-3.5" />
                  <span>{isSubmittingReview ? 'Signing...' : 'Sign & Record Assessment (21 CFR Part 11)'}</span>
                </button>
              </form>
            ) : (
      ...
      {isSignatureModalOpen && (
        <ElectronicSignatureModal
          ...
          actionType="SAFETY_MEDICAL_REVIEW_SIGN"
          actionTitle={`Medical Review: ${reviewStatus}`}
          entityId={event.ae_id}
          meaning={`I hereby confirm medical review and causal assessment of this adverse event as '${reviewCausality}' with status '${reviewStatus}'.`}
        />
      )}
    </div>
  );
