...
import type { ClinicalTrial, TrialSite, AuditLog, DocumentUploadResponse } from '../types';
...
interface UploadedTrialDocument {
  id: string;
  name: string;
  category: string;
  url: string;
...
export const TrialDetailPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'timeline' | 'protocol' | 'sites' | 'documents' | 'audit'>('overview');
  const [extraDocuments, setExtraDocuments] = useState<UploadedTrialDocument[]>([]);
  ...
  const handleExportTrialPdf = async () => {
    if (!trial) return;
    try {
      setIsExportingPdf(true);
      await generateClinicalReportPdf(
        `CLINICAL STUDY PROTOCOL DOSSIER — ${trial.trial_id}`,
        `${trial.trial_title} • Standard: ICH-GCP E6(R2) & CDSCO NDCTR 2019`,
        [
          {
            heading: '1. Study Protocol & Governance Details',
            keyValue: {
              'Trial Protocol ID': trial.trial_id,
              'Protocol Number': trial.protocol_number,
              'Protocol Version': `v${trial.protocol_version}`,
              'Study Phase': trial.study_phase,
              'Study Design': trial.study_design,
              'Regulatory Status': trial.status,
              'Principal Investigator': trial.principal_investigator?.full_name || 'Dr. Rajesh Sharma, MD (Ayurveda)',
              'Sponsor': trial.sponsor,
              'Target Participants': trial.target_participants,
              'Ayush Morbidity': trial.disease_condition,
              'NAMASTE Morbidity Code': trial.namaste_code || 'N/A',
              'WHO ICD-11 TM2 Code': trial.icd11_tm_code || 'N/A'
            }
          },
          {
            heading: '2. Multi-Centric Investigational Sites',
            table: {
              headers: ['Site ID', 'Center Name', 'Principal City', 'Target Quota', 'Status'],
              rows: (trial.sites || []).map((s) => [
                s.site_code,
                s.site_name,
                s.city,
                String(s.enrollment_target),
                s.site_status
              ])
            }
          },
          {
            heading: '3. Protocol Governance Milestones',
            table: {
              headers: ['Milestone Name', 'Planned Date', 'Status', 'GCP Compliance'],
              rows: (trial.milestones || []).map((m) => [
                m.milestone_name,
                m.planned_date,
                m.status,
                'Verified'
        ...
        `${trial.trial_id}_Protocol_Dossier.pdf`
      );
    } catch (err) {
      console.error('Failed to export trial dossier PDF:', err);
  ...
  const handleExportTrialJson = () => {
    if (!trial) return;
    exportDataToJson(trial, `${trial.trial_id}_Full_Specification.json`);
  };
  ...
  useEffect(() => {
    if (id === 'create' || id === 'new') {
      navigate('/trials/new', { replace: true });
      return;
    }
    loadTrial();
  }, [id]);
  ...
  if (isLoading) {
    return (
      <div className="p-16 text-center text-slate-500 text-sm">
        ...
        <p className="text-xs text-slate-400 mt-1">Retrieving verified clinical protocol and site data from database</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-12">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {trial.namaste_code && (
                <span
                  title="Ministry of Ayush National Ayush Morbidity & Standardized Terminology Electronic Portal (NAMASTE)"
                  className="inline-flex items-center space-x-1.5 text-xs font-semibold text-amber-900 bg-amber-50 px-2.5 py-0.5 rounded border border-amber-300 font-mono shadow-xs"
                >
                  <Tag className="w-3 h-3 text-amber-600" />
                  <span>NAMASTE: {trial.namaste_code}</span>
                </span>
              )}
              {trial.icd11_tm_code && (
                <span
                  title="WHO International Classification of Diseases 11th Revision - Traditional Medicine Chapter 2 (ICD-11 TM2)"
                  className="inline-flex items-center space-x-1.5 text-xs font-semibold text-sky-900 bg-sky-50 px-2.5 py-0.5 rounded border border-sky-300 font-mono shadow-xs"
                >
                  <BookOpen className="w-3 h-3 text-sky-600" />
                  <span>ICD-11 TM2: {trial.icd11_tm_code}</span>
                </span>
              )}
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2 self-start lg:self-auto">
            ...
            <button
              type="button"
              onClick={handleExportTrialPdf}
              disabled={isExportingPdf}
              className="inline-flex items-center space-x-1.5 px-3 py-2 text-xs font-semibold text-white bg-emerald-700 hover:bg-emerald-600 rounded-lg shadow-sm transition-colors disabled:opacity-50"
              title="Generate complete trial dossier PDF via HTML-to-Canvas"
            >
              {isExportingPdf ? (
            ...
            <button
              ...
              className="inline-flex items-center space-x-1.5 px-3 py-2 text-xs font-semibold text-slate-700 bg-slate-50 hover:bg-slate-100 border border-slate-300 rounded-lg shadow-sm transition-colors"
              title="Download structured study specification JSON"
            >
      ...
      <div className="flex border-b border-slate-200 bg-white rounded-t-xl overflow-x-auto text-xs font-semibold">
        ...
        >
          <FileCheck className="w-4 h-4 text-ayush-600" />
          <span>Dossiers & Source PDFs</span>
        </button>
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase tracking-wider font-semibold">
                    ...
                    <th className="py-3 px-4 text-right">Protocol File</th>
                  </tr>
                </thead>
                  ...
                  {trial.protocol_versions?.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-3 px-4 text-right">
                        <button
                          onClick={() => {
                            const fileUrl = p.document_url || `/api/v1/documents/protocol_${trial.trial_id}.pdf`;
                            setViewerUrl(fileUrl);
                            setViewerTitle(`${trial.trial_id}_Protocol_v${p.version_number}.pdf`);
                            setIsViewerOpen(true);
                          }}
                          className="inline-flex items-center space-x-1 px-2.5 py-1 text-[11px] font-semibold text-ayush-700 bg-ayush-50 hover:bg-ayush-100 rounded border border-ayush-200 transition-colors"
                        >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-slate-100">
              <div>
                <h3 className="text-base font-bold text-slate-900 flex items-center space-x-2">
                  ...
                  <span>Clinical Trial Dossiers & Source Documents</span>
                </h3>
                <p className="text-slate-500 text-xs mt-0.5">
                  Repository for verified study protocols, Investigator's Brochure, Subject Information Sheets, ICFs, and Ethics Approval letters.
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <span className="inline-flex items-center px-2 py-1 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded font-semibold text-[11px]">
            ...
            {canEdit && (
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
                <div className="flex items-center space-x-2 text-slate-800 font-bold">
                  <FileUp className="w-4 h-4 text-ayush-700" />
                  <span>Upload Protocol Amendment, ICF, or Regulatory Letter</span>
                </div>
                <DocumentUpload
                  accept=".pdf,.docx,.doc"
                  maxSizeMB={25}
                  onUploadSuccess={(resp: DocumentUploadResponse) => {
                    const newDoc: UploadedTrialDocument = {
                      id: resp.filename,
                      name: resp.original_filename,
                      category: 'Clinical Dossier / Amendment',
                      url: resp.url,
                      uploadedAt: new Date().toISOString(),
                      sizeMB: Math.round((resp.file_size / (1024 * 1024)) * 100) / 100
                    };
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase tracking-wider font-semibold">
                    ...
                    <th className="py-3 px-4">Version / Ref</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  ...
                  {extraDocuments.map((doc) => (
                    <tr key={doc.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-3.5 px-4 font-mono text-slate-500">Ad-hoc</td>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          <button
                            ...
                            className="inline-flex items-center space-x-1 px-2.5 py-1 text-xs font-semibold text-white bg-ayush-700 hover:bg-ayush-800 rounded shadow-xs transition-colors"
                          >
                  ...
                  {trial.protocol_versions?.map((p) => (
                    <tr key={`pver-${p.id}`} className="hover:bg-slate-50/60 transition-colors">
                      <td className="py-3.5 px-4 font-semibold text-slate-900">
                        <div className="flex items-center space-x-2">
                          <FileText className="w-4 h-4 text-indigo-600 flex-shrink-0" />
                          <span>{trial.protocol_number} — Clinical Study Protocol</span>
                        </div>
                      </td>
                      <td className="py-3.5 px-4 text-slate-600">Protocol Specification</td>
                      ...
                      <td className="py-3.5 px-4">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold border ${
                          ...
                          {p.status === 'Current' ? 'Active Approved' : p.status}
                        </span>
                      </td>
                      <td className="py-3.5 px-4 text-slate-500 font-mono">{p.version_date}</td>
                      <td className="py-3.5 px-4 text-right">
                        <div className="flex items-center justify-end space-x-2">
                          <button
                            onClick={() => {
                              const docUrl = p.document_url || `/api/v1/documents/protocol_${trial.trial_id}.pdf`;
                              setViewerUrl(docUrl);
                              setViewerTitle(`${trial.trial_id}_Protocol_v${p.version_number}.pdf`);
                              setIsViewerOpen(true);
                            }}
                  ...
                  {/* Standard Essential Regulatory Dossiers */}
                  <tr className="hover:bg-slate-50/60 transition-colors">
                    <td className="py-3.5 px-4 font-semibold text-slate-900">
                      <div className="flex items-center space-x-2">
                        <FileText className="w-4 h-4 text-amber-600 flex-shrink-0" />
                        <span>Informed Consent Form & Patient Information Sheet (Bilingual)</span>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 text-slate-600">ICF / Consent</td>
                    <td className="py-3.5 px-4 font-mono text-slate-800">ICF-v2.1-BIL</td>
                    <td className="py-3.5 px-4">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200">
                        IEC Approved
                      </span>
                    </td>
                    <td className="py-3.5 px-4 text-slate-500 font-mono">{trial.start_date}</td>
                    <td className="py-3.5 px-4 text-right">
                      <button
                        onClick={() => {
                          setViewerUrl(`/api/v1/documents/icf_${trial.trial_id}.pdf`);
                          setViewerTitle(`${trial.trial_id}_Informed_Consent_Sheet.pdf`);
                          setIsViewerOpen(true);
                        }}
                        className="inline-flex items-center space-x-1 px-2.5 py-1 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded transition-colors"
                      >
                        <Eye className="w-3.5 h-3.5" />
                        <span>Preview</span>
                      </button>
                  ...
                  <tr className="hover:bg-slate-50/60 transition-colors">
                    <td className="py-3.5 px-4 font-semibold text-slate-900">
                      <div className="flex items-center space-x-2">
                        ...
                        <span>Investigator's Brochure (IB) — Classical & Standardization Data</span>
                      </div>
                    </td>
                    <td className="py-3.5 px-4 text-slate-600">Investigator Brochure</td>
                    <td className="py-3.5 px-4 font-mono text-slate-800">IB-AYUSH-2026</td>
                    <td className="py-3.5 px-4">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200">
                        Standard Certified
                      </span>
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        onClick={() => {
                          setViewerUrl(`/api/v1/documents/ib_${trial.trial_id}.pdf`);
                          setViewerTitle(`${trial.trial_id}_Investigator_Brochure.pdf`);
                          setIsViewerOpen(true);
                        }}
                        className="inline-flex items-center space-x-1 px-2.5 py-1 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 rounded transition-colors"
                      >
