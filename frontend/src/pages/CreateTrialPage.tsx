...
import { NAMASTE_MORBIDITY_DATABASE } from '../data/namasteTerminology';
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-b-xl border border-slate-200 shadow-sm p-6 space-y-6">
        ...
        {activeTab === 'study' && (
          <div className="space-y-4">
            ...
            {/* Ministry of Ayush NAMASTE Portal Morbidity Selector */}
            <div className="bg-emerald-50/60 border border-emerald-200 rounded-lg p-3.5 space-y-2.5">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <span className="text-xs font-bold text-emerald-950">
                      Ministry of Ayush NAMASTE Portal Terminology
                    </span>
                    <span className="ml-2 text-[10px] bg-emerald-100 text-emerald-800 px-1.5 py-0.5 rounded font-mono">
                      ICD-11 Chapter 26 (TM2)
                    </span>
                  </div>
                </div>
                <span className="text-[10px] text-emerald-700 font-medium">Standardized Morbidity Registry</span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="sm:col-span-2">
                  <label className="block text-[11px] font-semibold text-slate-700 mb-1">
                    Select Standardized Ayush Condition (Auto-fills Terminology):
                  </label>
                  <select
                    value={formData.namaste_code || ''}
                    onChange={(e) => {
                      const selected = NAMASTE_MORBIDITY_DATABASE.find((c) => c.code === e.target.value);
                      if (selected) {
                        setFormData((prev) => ({
                          ...prev,
                          namaste_code: selected.code,
                          icd11_tm_code: selected.icd11Code,
                          disease_condition: `${selected.ayurvedaTerm} (${selected.englishTerm})`,
                          ayurveda_intervention: prev.ayurveda_intervention || selected.commonInterventions[0]
                        }));
                      } else {
                        setFormData((prev) => ({ ...prev, namaste_code: '', icd11_tm_code: '' }));
                      }
                    }}
                    className="w-full px-2.5 py-2 text-xs border border-emerald-300 rounded-lg bg-white focus:ring-2 focus:ring-emerald-600 focus:outline-hidden"
                  >
                    <option value="">-- Choose from NAMASTE Registry or type custom below --</option>
                    {NAMASTE_MORBIDITY_DATABASE.map((item) => (
                      <option key={item.code} value={item.code}>
                        {item.code}: {item.ayurvedaTerm} — {item.englishTerm} [{item.icd11Code}]
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-col space-y-1">
                    <span className="text-[11px] font-mono text-emerald-900 bg-white px-2 py-1 border border-emerald-200 rounded">
                      NAMASTE: <strong>{formData.namaste_code || 'Custom'}</strong>
                    </span>
                    <span className="text-[11px] font-mono text-emerald-900 bg-white px-2 py-1 border border-emerald-200 rounded">
                      ICD-11 TM: <strong>{formData.icd11_tm_code || 'Custom'}</strong>
                    </span>
                  </div>
                </div>
              </div>
              ...
              {formData.namaste_code && (
                <div className="pt-1 flex flex-wrap items-center gap-1.5 text-[11px] text-emerald-800">
                  <span className="font-semibold">Recommended Classical Formulations:</span>
                  {NAMASTE_MORBIDITY_DATABASE.find((c) => c.code === formData.namaste_code)?.commonInterventions.map((formulation) => (
                    <button
                      key={formulation}
                      type="button"
                      onClick={() => setFormData((prev) => ({ ...prev, ayurveda_intervention: formulation }))}
                      className="px-2 py-0.5 bg-white border border-emerald-300 text-emerald-700 hover:bg-emerald-100 rounded text-[10px] transition cursor-pointer"
                      title="Click to populate intervention name"
                    >
                      + {formulation}
                    </button>
                  ))}
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 text-xs border border-slate-300 rounded-lg focus:ring-2 focus:ring-ayush-600 focus:outline-hidden"
                required
              />
            </div>
