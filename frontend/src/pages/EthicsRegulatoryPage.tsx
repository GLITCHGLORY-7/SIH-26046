...
export const EthicsRegulatoryPage: React.FC = () => {
  ...
  const [regModalError, setRegModalError] = useState<string | null>(null);
  const [isSubmittingReg, setIsSubmittingReg] = useState(false);
  const [ethicsModalError, setEthicsModalError] = useState<string | null>(null);
  const [isSubmittingEthics, setIsSubmittingEthics] = useState(false);
  const [ctriModalError, setCtriModalError] = useState<string | null>(null);
  const [isSubmittingCtri, setIsSubmittingCtri] = useState(false);

  // Helper to format API error messages
  const formatApiError = (err: any, fallback = 'Operation failed'): string => {
  ...
  const handleCreateEthics = async (e: React.FormEvent) => {
    ...
    if (!ethicsForm.trial_id) {
      setEthicsModalError('Please select a target clinical trial');
      return;
    }
    setIsSubmittingEthics(true);
    setEthicsModalError(null);
    try {
      ...
      setEthicsModalError(formatApiError(err, 'Failed to submit ethics review request'));
    } finally {
      setIsSubmittingEthics(false);
    }
  };
  ...
  const handleCreateRegulatoryEvent = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!regForm.trial_id) {
      setRegModalError('Please select a target clinical trial');
      return;
    }
    ...
    try {
      ...
      setRegModalError(formatApiError(err, 'Failed to create regulatory milestone'));
    } finally {
  ...
  const handleUpdateCtri = async (e: React.FormEvent) => {
    ...
    setIsSubmittingCtri(true);
    setCtriModalError(null);
    try {
      ...
      setCtriModalError(formatApiError(err, 'Failed to update CTRI record'));
    } finally {
      setIsSubmittingCtri(false);
    }
  };
  ...
  const handleMarkRegEventCompleted = async (eventId: number) => {
    try {
      ...
      alert(formatApiError(err, 'Failed to update regulatory milestone'));
    }
  };
          ...
          {canManage && (
            <>
              ...
              <button
                onClick={() => {
                  setRegModalError(null);
                  setIsNewRegEventModalOpen(true);
                }}
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 space-y-4">
            ...
            <form onSubmit={handleCreateEthics} className="space-y-4 text-xs">
              ...
              <div className="flex justify-end space-x-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isSubmittingEthics}
                  className="px-4 py-1.5 bg-ayush-800 hover:bg-ayush-900 text-white rounded-lg font-semibold flex items-center space-x-2 disabled:opacity-50"
                >
                  {isSubmittingEthics && (
                    <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></div>
                  )}
                  <span>{isSubmittingEthics ? 'Submitting...' : 'Submit for IEC Review'}</span>
                </button>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={handleCreateRegulatoryEvent} className="space-y-4 text-xs">
              ...
              <div className="flex justify-end space-x-2 pt-2">
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-slate-800 hover:bg-slate-900 text-white rounded-lg font-semibold flex items-center space-x-2 disabled:opacity-50"
                >
                  {isSubmittingReg && (
                    <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></div>
                  )}
                  <span>{isSubmittingReg ? 'Adding...' : 'Add Milestone'}</span>
                </button>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full p-6 space-y-4">
            ...
            <form onSubmit={handleUpdateCtri} className="space-y-4 text-xs">
              ...
              <div className="flex justify-end space-x-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={isSubmittingCtri}
                  className="px-4 py-1.5 bg-blue-700 hover:bg-blue-800 text-white rounded-lg font-semibold flex items-center space-x-2 disabled:opacity-50"
                >
                  {isSubmittingCtri && (
                    <div className="w-3.5 h-3.5 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></div>
                  )}
                  <span>{isSubmittingCtri ? 'Saving...' : 'Save CTRI Data'}</span>
                </button>
              </div>
            </form>
          </div>
