...
export const SafetyPage: React.FC = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [eventsRes, statsRes, trialsRes, partsRes] = await Promise.allSettled([
        safetyService.listEvents(params),
        safetyService.getSummary(filterTrialId),
        trialService.getTrials({ limit: 100 }),
        participantService.listParticipants({ limit: 100 })
      ]);
      ...
      if (trialsRes.status === 'fulfilled') {
        ...
        if (trialsRes.value.length > 0 && modalTrialId === 0) {
          setModalTrialId(trialsRes.value[0].id);
        }
      }
  ...
  }, [filterTrialId, filterSeverity, filterSeriousness, filterStatus, modalTrialId]);
                      ...
                      <td className="py-3 px-4">
                        {isSAE ? (
                          <div className="space-y-1">
                            ...
                            {ae.expedited_notified_at ? (
                              <span className="block text-[10px] text-emerald-700 font-semibold font-mono">
                                ✓ DCGI Notified ({new Date(ae.expedited_notified_at).toLocaleDateString()})
                              </span>
                            ) : (
                              <span className="block text-[10px] text-amber-700 font-semibold animate-pulse font-mono">
                                ⚠ 24h Notice Pending
                              </span>
                            )}
                          </div>
              ...
              <div>
                ...
                >
                    ...
                    <option key={p.id} value={p.id}>
                      {p.participant_id} ({p.status || p.participant_status || 'Screened'}) — Trial #{p.trial_id}
                    </option>
                  ))}
                </select>
              </div>
