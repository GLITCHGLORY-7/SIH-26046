...
import type { EthicsSubmission, EthicsDecisionPayload, SignatureManifest, DocumentUploadResponse } from '../../types';
...
export const EthicsDecisionModal: React.FC<EthicsDecisionModalProps> = ({
  ...
  const [documentUrl, setDocumentUrl] = useState<string>(submission.document_url || '');
  ...
  const handleInitiateSignature = (e: React.FormEvent) => {
    e.preventDefault();
    if (decision === 'Approved' && (!approvalNumber || !approvalDate || !expiryDate)) {
      setError('Please provide all approval certificate dates and reference number.');
      return;
    }
    setError(null);
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full border border-gray-100 overflow-hidden animate-fadeIn">
        ...
        <form onSubmit={handleInitiateSignature} className="p-6 space-y-4">
          {error && (
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Signed IEC Clearance Certificate / Approval Letter (PDF)
            </label>
            <DocumentUpload
              accept=".pdf,.docx,.doc"
              maxSizeMB={20}
              onUploadSuccess={(resp: DocumentUploadResponse) => setDocumentUrl(resp.url)}
            />
          </div>
          ...
          <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-[11px] text-slate-600 flex items-center space-x-2">
            <Lock className="w-4 h-4 text-emerald-700 flex-shrink-0" />
            <span>
              21 CFR Part 11 Electronic Signature required upon submission to create cryptographic manifest and tamper-evident audit record.
            </span>
          </div>
          ...
          <div className="flex justify-end space-x-3 pt-3 border-t">
            ...
            <button
              ...
              className="inline-flex items-center space-x-2 px-5 py-2 text-sm font-semibold text-white bg-emerald-800 hover:bg-emerald-900 rounded-lg shadow transition disabled:opacity-50"
            >
              <Lock className="w-4 h-4 text-emerald-300" />
              <span>{isSubmitting ? 'Signing & Recording...' : 'Sign & Commit Decision'}</span>
            </button>
          </div>
        </form>
      </div>
      ...
      {isSignatureModalOpen && (
        <ElectronicSignatureModal
          ...
          actionType="ETHICS_DECISION_SIGN"
          actionTitle={`IEC Ethics Decision: ${decision}`}
          entityId={submission.submission_id || String(submission.id)}
          meaning={`Official Ethics Committee review and ${decision} determination under 21 CFR Part 11.`}
        />
      )}
    </div>
  );
