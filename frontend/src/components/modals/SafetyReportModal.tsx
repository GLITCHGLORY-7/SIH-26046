...
export const SafetyReportModal: React.FC<SafetyReportModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let errorMsg = 'Failed to report safety event.';
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="flex justify-end space-x-3 pt-3 border-t">
            ...
            <button
              ...
              className="px-5 py-2 text-sm font-semibold text-white bg-amber-700 hover:bg-amber-800 rounded-lg shadow transition disabled:opacity-50 flex items-center space-x-2"
            >
              {isSubmitting && (
                <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin mr-1"></div>
              )}
              <span>{isSubmitting ? 'Submitting...' : 'Submit Safety Report'}</span>
            </button>
          </div>
        </form>
      </div>
